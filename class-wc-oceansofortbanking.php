...
/**
 * Oceanpayment Sofortbanking Payment Gateway
 *
 * Provides a Oceanpayment Sofortbanking Payment Gateway, mainly for testing purposes.
 *
 * @class 		WC_Gateway_Oceansofortbanking
 * @extends		WC_Payment_Gateway
 * @version		1.2
 * @package		WooCommerce/Classes/Payment
 * @author 		Oceanpayment
 */
class WC_Gateway_Oceansofortbanking extends WC_Payment_Gateway {
    ...
    public function __construct() {
        $this->id                 = 'oceansofortbanking';
        $this->icon               = apply_filters('woocommerce_oceansofortbanking_icon', plugins_url( 'images/op_Sofortbanking.png', __FILE__ ));
        $this->has_fields         = false;
        $this->method_title       = __( 'Oceanpayment Sofortbanking', 'woocommerce' );
        ...
        // Actions
        add_action( 'woocommerce_api_wc_gateway_oceansofortbanking', array( $this, 'check_ipn_response' ) );
        add_action( 'woocommerce_update_options_payment_gateways_' . $this->id, array( $this, 'process_admin_options' ) );
        add_action( 'valid-oceansofortbanking-standard-itn-request', array( $this, 'successful_request' ) );
        add_action( 'woocommerce_receipt_' . $this->id, array( $this, 'receipt_page' ) );
        add_action( 'woocommerce_thankyou_oceansofortbanking', array( $this, 'thankyou_page' ) );
        ...
        add_action( 'woocommerce_receipt_oceansofortbanking', array( $this, 'receipt_page' ) );
    }
    ...
    public function init_form_fields() {
        ...
        $this->form_fields = array(
            'enabled' => array(
                ...
                'type'    => 'checkbox',
                'label'   => __( 'Enable Oceanpayment Sofortbanking Payment', 'woocommerce' ),
                'default' => 'yes'
            ),
            'title' => array(
                ...
                'default'     => __( 'Sofortbanking Payment', 'woocommerce' ),
                'desc_tip'    => true,
            ),
            'description' => array(
	...
	public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
		if ( $this->instructions && ! $sent_to_admin && 'oceansofortbanking' === $order->get_payment_method() && $order->has_status( 'on-hold' ) ) {
    ...
    function receipt_page( $order ) {
        echo $this->generate_sofortbanking_form( $order );
    }
    ...
    /**
     * 生成 Sofortbanking form.
     */
    public function generate_sofortbanking_form( $order_id ) {
        ...
        //账单人州(可不提交)
        $billing_state     = $this->get_sofortbanking_state( $order->get_billing_country(), $order->get_billing_state() );
        //账单人城市
        ...
        //收货人的州（省、郡）
        $ship_state 	   = $this->get_sofortbanking_state( $order->get_shipping_country(), $order->get_shipping_state() );
        //收货人的城市
        ...
        $data_to_send .= "</div>";
        $data_to_send .= "</div>";
        $data_to_send .= "<form  method='post' name='sofortbanking_checkout' action='".$this->settings['submiturl']."'  >";
        ...
        if($this->settings['mode'] == 'redirect'){
            ...
            $data_to_send .= 'document.sofortbanking_checkout.submit();' . "\n";
            $data_to_send .= '</script>' . "\n";

        }elseif($this->settings['mode'] == 'iframe'){
            $data_to_send .= '<iframe width="100%" height="800px"  scrolling="auto" style="border:none ; margin: 0 auto; overflow:hidden;" id="ifrm_sofortbanking_checkout" name="ifrm_sofortbanking_checkout"></iframe>' . "\n";

            $data_to_send .= '<script type="text/javascript">' . "\n";
            $data_to_send .= 'document.sofortbanking_checkout.target="ifrm_sofortbanking_checkout";' . "\n";
            $data_to_send .= 'document.sofortbanking_checkout.submit();' . "\n";
            $data_to_send .= 'var ifrm_cc  = document.getElementById("ifrm_sofortbanking_checkout");' . "\n";
            ...
            if($return_info['response_type'] == 1){
                ...
                if(strtoupper($local_signValue) == strtoupper($return_info['signValue'])){
                    ...
                    if ($return_info['payment_status'] == 1) {
                        //成功
                        $order->update_status( 'processing', __( $testorder.$return_info['payment_details'], 'oceanpayment-sofortbanking-gateway' ) );
                       wc_reduce_stock_levels( $order_id );
                        ...
                        if(empty($this->completed_orders()) || !in_array($return_info['order_number'], $this->completed_orders())){
                    		$order->update_status( 'on-hold', __( $testorder.$return_info['payment_details'], 'oceanpayment-sofortbanking-gateway' ) );
                		}
                        ...
                        if($errorCode == '20061'){
                            ...
                            if(empty($this->completed_orders()) || !in_array($return_info['order_number'], $this->completed_orders())){
                    			$order->update_status( 'failed', __( $testorder.$return_info['payment_details'], 'oceanpayment-sofortbanking-gateway' ) );
                			}
                        }
     ...
     * 获取州/省
     */
    public function get_sofortbanking_state( $cc, $state ) {
    ...
    public function postLog($data, $logType){
        ...
        $post_log .= 'log'.$this->settings['log'];
        $post_log = $post_log . "*************************************\r\n";
    ...
    {
        $html = '<script type="text/javascript">parent.location.replace("'.$url.'");</script>';
